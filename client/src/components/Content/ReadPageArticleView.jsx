...
import { FaThumbsUp, FaRegThumbsUp, FaReply, FaSpinner, FaFlag, FaRegFlag, FaListUl, FaTimes, FaEdit, FaSave, FaWindowClose, FaBookmark, FaRegBookmark } from 'react-icons/fa';
...
const LineageSegmentDisplay = ({ content, color, onSegmentClick, isActiveForActions }) => {
    ...
    return (
        <span
            ...
            style={{ backgroundColor: color, border: isActiveForActions ? '3px solid #0056b3' : '1px solid rgba(0,0,0,0.1)', padding: '8px 12px', margin: '4px 2px', display: 'inline-block', borderRadius: '4px', cursor: 'pointer', transition: 'all 0.2s ease', boxShadow: isActiveForActions ? '0 0 8px rgba(0,86,179,0.3)' : '0 1px 2px rgba(0,0,0,0.05)', transform: isActiveForActions ? 'scale(1.01)' : 'scale(1)', }}
...
const ReadPageArticleView = ({ initialLineage: propInitialLineage, rootArticleId: propRootArticleId, onLineageUpdateFromParent }) => {
  ...
  const checkIfLineageIsSaved = useCallback(async (lineageToCheck) => { if (!isAuthenticated || !lineageToCheck || lineageToCheck.length === 0) { setIsCurrentLineageSaved(false); return; } try { const { data: savedItems } = await apiClient.get('/users/me/saved-articles'); const currentPathIdsString = lineageToCheck.map(s => s.id).join(','); const found = savedItems.some(item => item.lineagePathIds?.map(p => typeof p === 'string' ? p : p._id).join(',') === currentPathIdsString); setIsCurrentLineageSaved(found); } catch (err) { console.error("Error checking if lineage is saved:", err); setIsCurrentLineageSaved(false); } }, [apiClient, isAuthenticated]);
    ...
    try {
        ...
        if (passedPathIds && passedPathIds.length > 0) {
            ...
            if (reconstructedPath.length > 0) {
                ...
                if(reconstructedPath.length !== passedPathIds.length) setError("Note: Some parts of the saved lineage could not be loaded.");
  ...
  const refreshLineageAndSetActive = useCallback((actedUponSegmentId = null) => { if (!rootArticleId) return; setLoading(true); clearActionMessages(); apiClient.get(`/content/${rootArticleId}/lineage`).then(response => { const refreshedLineage = (response.data || []).filter(s => s && s.id && typeof s.text === 'string'); setCurrentLineage(refreshedLineage); if (onLineageUpdateFromParent) onLineageUpdateFromParent(refreshedLineage); if (actedUponSegmentId) { const newActive = refreshedLineage.find(s => s.id === actedUponSegmentId); setActiveSegmentForActions(newActive || null);}}).catch(err => { console.error("Error refreshing lineage:", err); setError("Could not refresh content."); }).finally(() => setLoading(false)); }, [apiClient, rootArticleId, onLineageUpdateFromParent, clearActionMessages]);
  const handleLikeActiveSegment = async () => { if (!activeSegmentForActions || !isAuthenticated) return; setActionLoading({ type: 'like', id: activeSegmentForActions.id }); clearActionMessages(); try { const { data } = await apiClient.post(`/content/${activeSegmentForActions.id}/like`); const updatedSegment = { ...activeSegmentForActions, likeCount: data.likeCount, likes: data.likes }; setActiveSegmentForActions(updatedSegment); handleSegmentDataChangeInLineage(updatedSegment); } catch (err) { setActionError(err.response?.data?.error || "Like failed."); } finally { setActionLoading({ type: null, id: null }); } };
  
  // --- CORRECTED TOGGLE REPORT LOGIC ---
  const handleToggleReportActiveSegment = async () => {
    ...
    try {
        ...
        if (currentUserHasReported) {
            ...
            // User wants to SUBMIT a report
            response = await apiClient.post(`/content/${activeSegmentForActions.id}/report`, { reason: "Reported from active segment actions" });
            setActionSuccess("Segment reported successfully.");
        }
        
        const { data } = response;
        // The backend should return the updated state
        const updatedSegment = {
            ...activeSegmentForActions,
            isReported: data.isReported,
            reportsCount: data.reportsCount,
            // We can manually update the reports array for immediate UI feedback
            reports: currentUserHasReported ?
                     (activeSegmentForActions.reports || []).filter(r => (r.reporter !== user.id && r.reporter?._id !== user.id)) :
                     [...(activeSegmentForActions.reports || []), {reporter: user.id}] // Simplified local update
        };
        ...
        setTimeout(clearActionMessages, 4000); // Clear messages after a delay
    }
  };
  // --- END OF CORRECTED LOGIC ---

  const handleReplyToActiveSegmentSuccess = (newReply) => { setIsReplyingToActiveSegment(false); setActionSuccess("Reply posted!"); refreshLineageAndSetActive(newReply.parentContent); };
  const handleEditActiveSegmentSuccess = (updatedContent) => { setIsEditingActiveSegment(false); setActiveSegmentForActions(updatedContent); handleSegmentDataChangeInLineage(updatedContent); setActionSuccess("Segment updated!"); };
  const handleSegmentDataChangeInLineage = (updatedSegmentData) => { const updatedLineage = currentLineage.map(segment => segment.id === updatedSegmentData.id ? { ...segment, ...updatedSegmentData } : segment ); setCurrentLineage(updatedLineage); if (onLineageUpdateFromParent) onLineageUpdateFromParent(updatedLineage); if (updatedSegmentData.action?.includes('versions')) { refreshLineageAndSetActive(updatedSegmentData.id); } };
  const handleShowSiblingVersions = (segmentIdClicked, show) => { const segmentObject = currentLineage.find(s => s.id === segmentIdClicked); if (show && segmentObject && segmentObject.parentContent) { setShowVersionSelectorFor(segmentObject); setIsReplyingToActiveSegment(false); setIsEditingActiveSegment(false); setActiveSegmentForActions(null); clearActionMessages(); } else { setShowVersionSelectorFor(null); } };
  const handleSelectSiblingVersionForLineage = async (selectedSiblingId) => { if (!showVersionSelectorFor) return setError("Error: Context missing."); const originalDepth = currentLineage.findIndex(s => s.id === showVersionSelectorFor.id); if (originalDepth === -1) return setError("Error: Depth not found."); setLoading(true); setError(null); clearActionMessages(); setShowVersionSelectorFor(null); try { const { data: newPartialLineageResponse } = await apiClient.get(`/content/${selectedSiblingId}/lineage`); const newPartialLineage = (newPartialLineageResponse || []).filter(s => s && s.id && typeof s.text === 'string'); if (newPartialLineage.length === 0) throw new Error("Could not construct path."); const newLineage = [ ...currentLineage.slice(0, originalDepth), ...newPartialLineage ]; setCurrentLineage(newLineage); setActiveSegmentForActions(newPartialLineage[0]); if (onLineageUpdateFromParent) onLineageUpdateFromParent(newLineage); } catch (err) { console.error("Failed to update lineage:", err); setError(err.response?.data?.error || "Failed to load lineage."); } finally { setLoading(false); } };
  const handleSaveLineage = async () => { if (!isAuthenticated || !currentLineage || currentLineage.length === 0) { alert("Please log in to save."); return; } const rootId = currentLineage[0].id; const lineagePathIdsToSend = currentLineage.map(segment => segment.id); setActionLoading({ type: 'save', id: rootId }); setSaveStatus({ message: '', type: '' }); clearActionMessages(); try { await apiClient.post('/users/me/saved-articles', { rootArticleId: rootId, lineagePathIds: lineagePathIdsToSend }); setSaveStatus({ message: 'Lineage saved!', type: 'success' }); setIsCurrentLineageSaved(true); } catch (err) { console.error("Failed to save lineage:", err); setSaveStatus({ message: err.response?.data?.error || "Failed to save.", type: 'error' }); } finally { setActionLoading({ type: null, id: null }); } };
  ...
  return (
    <div className="read-page-article-view">
      ...
      <div className="lineage-metadata card-meta" style={{ marginBottom: '20px', padding: '10px', background: '#f8f9fa', borderRadius: '4px', border:'1px solid #eee' }}>
          <p style={{margin:0}}> <strong>Author:</strong> {rootDisplayArticle.author?.username || 'Unknown'} |{' '} <strong>Posted:</strong> {format(new Date(rootDisplayArticle.createdAt || Date.now()), 'PPP p')} |{' '} <strong>Likes:</strong> <FaThumbsUp size="0.8em" style={{verticalAlign: 'baseline'}}/> {rootDisplayArticle.likeCount || 0} </p>
      ...
      {activeSegmentForActions && !showVersionSelectorFor && (
        <div className="active-segment-actions-panel card" style={{padding: '15px', marginBottom: '20px', background:'#fff', border:'2px solid #007bff'}}>
            ...
            {actionError && <p className="error-message" style={{marginBottom:'10px'}}>{actionError}</p>}
            {actionSuccess && <p className="success-message" style={{marginBottom:'10px'}}>{actionSuccess}</p>}
            <div style={{display:'flex', flexWrap:'wrap', gap:'10px', marginBottom: isReplyingToActiveSegment || isEditingActiveSegment ? '15px' : '0'}}>
                ...
                {isAuthenticated && (<button onClick={() => { setIsReplyingToActiveSegment(true); setIsEditingActiveSegment(false); clearActionMessages(); }} className="btn btn-sm btn-outline-secondary" disabled={isReplyingToActiveSegment}> <FaReply/> Reply </button> )}
                ...
                {activeSegmentForActions.parentContent && (<button onClick={() => {setShowVersionSelectorFor(activeSegmentForActions); setActiveSegmentForActions(null); clearActionMessages();}} className="btn btn-sm btn-outline-success"> <FaListUl/> View/Manage Sibling Versions </button> )}
            </div>
