...
const VersionSelector = ({
  ...
  const [actionsOpenForSibling, setActionsOpenForSibling] = useState(null); // ID of SIBLING whose actions panel is open
  const [showNewVariationFormForSibling, setShowNewVariationFormForSibling] = useState(null); // For SIBLING
  const [showReplyFormForContext, setShowReplyFormForContext] = useState(false); // Reply to contextSegment

  // Local state for the contextSegment's like/report status
  ...
  useEffect(() => {
    // Update local states when the contextSegment prop changes
    if (contextSegment) {
  ...
  const fetchSiblingVersions = useCallback(async () => {
    if (!contextSegment || !contextSegment.id) {
        setSiblingVersions([]);
        setLoadingSiblings(false); // Ensure loading is false if no context
        return;
    }
    ...
    // setShowReplyFormForContext(false); // Keep reply form for context open if it was already
  ...
  const handleSiblingVersionItemClick = (siblingId) => {
    setActionsOpenForSibling(prev => (prev === siblingId ? null : siblingId));
    setShowNewVariationFormForSibling(null);
  };
  const handleToggleLikeSibling = async (siblingId, e) => { /* ... same as before ... */
    e.stopPropagation();
  ...
  const handleReportSibling = async (siblingId, e) => { /* ... same as before ... */
    e.stopPropagation();
  ...
  const handleToggleNewVariationFormForSibling = (contextForForm, e) => { /* ... same as before ... */
    if(e) e.stopPropagation();
    setShowNewVariationFormForSibling(prev => (prev === contextForForm ? null : contextForForm));
  };
  const handleNewSiblingVariationPostSuccess = (newVariation) => { /* ... same as before ... */
    setShowNewVariationFormForSibling(null);
    fetchSiblingVersions();
    if (onNewVariationAdded) onNewVariationAdded(newVariation);
  };
  const handleUseThisSiblingVersion = (siblingIdToUse, e) => { /* ... same as before ... */
    e.stopPropagation();
    if (onSelectVersion) onSelectVersion(siblingIdToUse);
  };

  if (!contextSegment) {
    return <div className="card" style={{padding: '1rem', marginTop: '1rem', textAlign:'center'}}>Select a segment in the main lineage to manage its versions.</div>;
  }
  ...
  return (
    <div className="version-selector card" style={{marginTop: '1rem', background: '#f0f2f5', border:'1px solid #007bff', padding: '15px'}}>
      ...
      {/* Actions for the CONTEXT SEGMENT - MOVED HERE, ABOVE SIBLING LIST */}
