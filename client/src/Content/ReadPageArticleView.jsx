...
const ReadPageArticleView = ({ initialLineage: propInitialLineage, rootArticleId: propRootArticleId, onLineageUpdateFromParent }) => {
  ...
  const handleReplyToActiveSegmentSuccess = useCallback((newReply) => { setIsReplyingToActiveSegment(false); setActionSuccess("Reply posted!"); const parentId = newReply.parentContent; const parentIndex = currentLineage.findIndex(segment => segment.id === parentId); if (parentIndex > -1) { const newPath = [...currentLineage.slice(0, parentIndex + 1), newReply]; setCurrentLineage(newPath); if (onLineageUpdateFromParent) onLineageUpdateFromParent(newPath); setActiveSegmentForActions(newReply); } else { refreshLineageAndSetActive(parentId); } setTimeout(clearActionMessages, 4000); }, [currentLineage, onLineageUpdateFromParent, refreshLineageAndSetActive, clearActionMessages]);
  ...
  // --- NEW: Determine which segment's data to display in the metadata section ---
  // If a segment is selected for actions, use its data. Otherwise, use the root article's data.
    ...
    <div className="read-page-article-view">
      ...
      <div className="concatenated-lineage-display card" style={{padding: '15px', marginBottom: '20px', background:'#fff', lineHeight: '1.6', fontSize: '1.1rem'}}>
        {currentLineage.map((content, index) => {
          ...
          return ( <LineageSegmentDisplay key={content.id + '-' + content.updatedAt} content={content} color={segmentColors[index % segmentColors.length]} onSegmentClick={handleSegmentClickForActionsPanel} isActiveForActions={activeSegmentForActions?.id === content.id} /> );
        })}
      </div>

      {/* --- UPDATED: Metadata section now uses `displaySegment` --- */}
